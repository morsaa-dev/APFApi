    ...
    public class RolesController : BaseApiController
    {
        ...
        public async Task<IHttpActionResult> ManageUsersInRole(UsersInRoleModel model)
        {
            ...
            foreach (string user in model.EnrolledUsers)
            {
                ...
                if (appUser == null)
                {
                    ModelState.AddModelError("error", String.Format("El usuario: {0} no existe.", user));
                    continue;
                }
                ...
                if (!this.AppUserManager.IsInRole(user, role.Name))
                {
                    ...
                    if (!result.Succeeded)
                    {
                        ModelState.AddModelError("error", String.Format("El usuario: {0} no puede ser añadido al rol.", user));
                    }
            ...
            foreach (string user in model.RemovedUsers)
            {
                ...
                if (!result.Succeeded)
                {
                    ModelState.AddModelError("error", String.Format("El usuario: {0} no puede ser removido del rol.", user));
                }
            }
